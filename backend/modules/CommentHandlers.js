  ...
  pool.getConnection((err, connection) => {
    ...
    connection.query(
      ...
      (err, recipe) => {
        ...
        connection.query(
          `SELECT COMMENT_ID, RECIPE_ID, COMMENTER, FIRST_NAME, LAST_NAME, COMMENT_CONTENT, LIKES FROM Receiply.Comments INNER JOIN Receiply.Accounts ON Comments.COMMENTER = Accounts.USER_ID WHERE RECIPE_ID='${RECIPE_ID}'`,
          (err, data) => {
            if (err) throw err;
            if (data.length === 0) {
              res.send(false).end();
