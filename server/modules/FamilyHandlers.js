                  ...
                  connection.query(
                    ...
                    (err, data) => {
                      if (err) {
                        ...
                        connection.query(
                          `UPDATE Accounts SET FAMILY = '${data[0].FAMILY_ID}', FAMILY_AUTH = 1 WHERE USER_ID= '${result.user_id}'`,
                          (err, data) => {
                            if (err) {
                              console.log(err);
                            } else {
        ...
        (err, data) => {
          ...
          connection.query(
            `SELECT a.FAMILY_ID, a.FAMILY_NAME, a.FAMILY_IDENTIFIER, a.FAMILY_CREATOR, Accounts.FIRST_NAME, 
              Accounts.LAST_NAME FROM (SELECT FAMILY_ID, FAMILY_NAME, FAMILY_CREATOR, FAMILY_IDENTIFIER FROM Families 
              WHERE Families.FAMILY_NAME LIKE '%${lastName}%') a INNER JOIN Accounts ON a.FAMILY_CREATOR=Accounts.USER_ID`,
            (err, family) => {
              if (err) throw err;
              if (family.length === 0) {
                res.end();
              ...
              (err, data) => {
                if (err) {
                  ...
                  connection.query(
                    ...
                    (err, data) => {
                      if (err) {
                        res.send("3rd query error!");
                      } else {
                        connection.query(
                          `UPDATE receiply_receiply.Accounts SET FAMILY = '${data[0].FAMILY_ID}', FAMILY_AUTH = 1 WHERE USER_ID= '${result.user_id}'`,
                          (err, data) => {
                            if (err) {
                              res.send("error!").end();
                            } else {
      ...
      connection.query(
        ...
        (err, data) => {
          ...
          if (data[0].VERIFIED === 0) {
            ...
            const familyID = req.body.FAMILY_ID;
            connection.query(
              `UPDATE receiply_receiply.Accounts SET FAMILY = '${familyID}', FAMILY_AUTH = 'request' WHERE USER_ID= '${userID}'`,
              (err, data) => {
                if (err) throw err;
                res.send(true);
                res.end();
    ...
    jwt.verify(token, process.env.ACCESS_TOKEN_KEY, (err, result) => {
      ...
      connection.query(
        ...
        (err, data) => {
          ...
          connection.query(
            `SELECT a.FAMILY_ID, a.FAMILY_NAME, a.FAMILY_IDENTIFIER, a.FAMILY_CREATOR, Accounts.FIRST_NAME, 
              Accounts.LAST_NAME FROM (SELECT FAMILY_ID, FAMILY_NAME, FAMILY_CREATOR, FAMILY_IDENTIFIER FROM receiply_receiply.Families 
              WHERE Families.FAMILY_NAME LIKE '%${lastName}%') a INNER JOIN receiply_receiply.Accounts ON a.FAMILY_CREATOR=Accounts.USER_ID`,
            (err, family) => {
              if (err) throw err;
              if (family.length === 0) {
                res.end();
