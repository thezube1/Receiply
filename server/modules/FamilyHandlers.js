      ...
      (err, token) => {
        ...
        connection.query(
          ...
          (err, result1) => {
            ...
            connection.query(
              `SELECT FAMILY_CREATOR FROM Families WHERE FAMILY_ID='${family}'`,
              (err, data) => {
                if (err) return console.log(err);
                else {
                  connection.query(
                    `SELECT USERNAME, FIRST_NAME, LAST_NAME, USER_ID FROM Accounts WHERE FAMILY='${family}' AND FAMILY_AUTH='1'`,
                    (err, result2) => {
                      if (err) return console.log(err);
                      else {
                        res.json({
                          CREATOR: data[0].FAMILY_CREATOR,
                          DATA: result2,
                        });
                        res.end();
                      }
